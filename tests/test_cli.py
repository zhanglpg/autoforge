"""Tests for autoforge.__main__ — CLI argument parsing and command dispatch."""
...
class TestCmdHealth:
    def test_health_json_in_process(self, capsys):
        """In-process health check with JSON format."""
    ...
    def test_health_text_in_process(self, capsys):
        with tempfile.TemporaryDirectory() as d:
    ...
    def test_health_save_to_file(self, capsys):
        with tempfile.TemporaryDirectory() as d:
            ...
            assert "Health Report" in Path(output_file).read_text()
            captured = capsys.readouterr()
            assert "Saved to" in captured.out

    def test_health_no_metrics_returns_error(self, capsys):
        """Non-existent adapter should return 1."""
        with tempfile.TemporaryDirectory() as d:
            parser = build_parser()
    ...
    def test_health_specific_adapter(self, capsys):
        with tempfile.TemporaryDirectory() as d:
            (Path(d) / "example.py").write_text("def f(): pass\n")
            parser = build_parser()
            args = parser.parse_args(["health", "--path", d, "--repo", d, "--adapters", "complexity", "--format", "json"])
            ret = cmd_health(args)
            assert ret == 0
            captured = capsys.readouterr()
            data = json.loads(captured.out)
    ...
    def test_health_adapter_prereq_fails(self, capsys):
        """When adapter prerequisites fail, it's skipped gracefully."""
        with tempfile.TemporaryDirectory() as d:
            (Path(d) / "example.py").write_text("def f(): pass\n")
            with patch("autoforge.registry.get_adapter") as mock_get:
                mock_adapter = MagicMock()
                mock_adapter.check_prerequisites.return_value = False
                mock_get.return_value = mock_adapter
                parser = build_parser()
                args = parser.parse_args(["health", "--path", d, "--repo", d, "--adapters", "complexity"])
                ret = cmd_health(args)
                assert ret == 1  # no metrics collected


class TestCmdRun:
    def test_run_dry_run_in_process(self, capsys):
        """In-process dry run of cmd_run."""
        with tempfile.TemporaryDirectory() as d:
    ...
    def test_run_with_budget_overrides(self, capsys):
        with tempfile.TemporaryDirectory() as d:
            ...
            args = parser.parse_args([
                ...
                "--path", d, "--repo", d,
                "--skip-git", "--skip-tests", "--dry-run",
                "--max-iterations", "2", "--max-tokens", "999", "--max-time", "1",
                "--output", output_dir,
            ])
            ret = cmd_run(args)
            assert ret in (0, 1)

    def test_run_with_explicit_adapter(self, capsys):
        with tempfile.TemporaryDirectory() as d:
            (Path(d) / "x.py").write_text("def f(): return 1\n")
            output_dir = str(Path(d) / "reports")
            parser = build_parser()
